...
void ControlPanel::setSong(const MPDSong &s) {
	if (s.isNull()) {
		titleLabel->setText(MPDConnection::instance()->isConnected() ? "" : QString("<h3>%1</h3>").arg(
			tr("Not connected", "qmpdclient is not connected to MPD")));
		artistLabel->setText("");
	...
	const int albumWidth = isVisible() ? albumLabel->width() : width() - 200;

	QString title = s.title();
	QString titleRich = elideRichText("<h3>", title, "</h3>", titleWidth);
	QString artist = s.artist();
	QString artistRich = elideRichText("<b>", artist, "</b>", artistWidth);
	QString album = s.album();
	QString albumRich = elideRichText("<b>", album, "</b>", albumWidth);

	titleLabel->setText(titleRich);
	titleLabel->setToolTip(title);
	artistLabel->setText(artistRich);
	artistLabel->setToolTip(artist);
	albumLabel->setText(albumRich);
